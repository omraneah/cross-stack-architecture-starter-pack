...
import { CognitoIdentityServiceProvider } from 'aws-sdk';  // ← in order.service.ts
const user = await okta.getUser({ AccessToken: token }).promise();
...
const role = decodedToken['okta:groups'][0];  // ← role from provider, not DB
...
return { userId: user.id, bookingId: order.id };  // ← no providerUserId ✓
```
