...
**Detection heuristic:** Search for any provider-specific identifier outside of `auth/`, `user/`, and `external-services/` directories.
...
export class ResourceEntity {
  ...
  providerUserId: string;  // ← provider identifier in a business table
}

// VIOLATION 2: Provider SDK imported in a business service
import { ProviderSdk } from 'provider-sdk';  // ← in resource.service.ts
const user = await providerClient.getUser({ AccessToken: token });

// VIOLATION 3: Provider identifier in an API response
return {
  userId: user.id,
  providerUserId: user.providerUserId,  // ← exposed to client
  resourceId: resource.id,
};

// VIOLATION 4: Role read from provider claims
const role = decodedToken.groups[0];  // ← role from provider, not DB

// VIOLATION 5: Provider identifier in a business query filter
const user = await this.userRepo.findOne({ where: { providerUserId } });
// (Acceptable ONLY in the user/auth module's provider-mapping table — never elsewhere)
```
...
**At provider migration time:** Every business table and service that references the provider ID must be audited and rewritten. A migration affects every row in every affected table. At scale, this is a multi-team, multi-month effort.
...
return { userId: user.id, resourceId: resource.id };  // ← no providerUserId
```
...
- Any field name matching provider identifier patterns
