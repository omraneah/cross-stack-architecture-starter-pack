...
```
**Correction:** TripService emits a `trip.status.changed` event when it does its work. BookingModule subscribes to that event and reacts. Neither module imports the other's services.
...
```typescript
// WRONG — handler has to call back into booking module
