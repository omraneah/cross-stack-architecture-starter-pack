...
- **An application protocol** (`APPLY-WITH-AI.md`). How to use this pack — with a human or with an AI agent — to audit an existing codebase or to bootstrap a new one. Includes severity scale, output template, and the rule that boundaries whose `Applies when:` predicate doesn't match are skipped, not partially graded.
...
2. `APPLY-WITH-AI.md` — the application protocol for audits and bootstraps.
